...
export const getDashboardStats = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const results = await Promise.all([
      ...
      Invoice.find({ businessAdminId: businessAdminId as any }).sort({ createdAt: -1 }).limit(10).select('invoiceNumber customerName grandTotal paymentStatus createdAt').lean(),
