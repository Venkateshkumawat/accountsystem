    ...
    <div className="space-y-4 select-none print:m-0 print:p-0">
        ...
        <div className="lg:col-span-8 bg-white p-5 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              <span className="text-[9px] font-semibold text-indigo-600 bg-indigo-50 px-2 py-1 rounded">LIVE_FEED</span>
            </div>
          </div>
              ...
              {salesData?.topSoldItems?.length > 0 ? salesData.topSoldItems.slice(0, 3).map((item: any, i: number) => (
                <div key={i} className="flex justify-between items-center text-[11px] py-1 border-b border-white/5 last:border-b-0 leading-none">
                  <div className="flex items-center gap-2">
                    <span className="text-white/20 font-semibold">#{i + 1}</span>
                    <span className="font-semibold text-white/90 uppercase truncate max-w-[100px]">{item._id}</span>
                  </div>
                  <span className="font-semibold text-indigo-400">₹{item.revenue.toLocaleString()}</span>
                </div>
              )) : <div className="py-10 text-center"><p className="text-[10px] text-white/10 uppercase font-semibold tracking-widest">NODATA_SYNC</p></div>}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {salesData?.lowStockItems?.length > 0 ? salesData.lowStockItems.slice(0, 3).map((item: any, i: number) => (
                <div key={i} className="flex justify-between items-center text-[11px] py-1 border-b border-slate-50 last:border-b-0 leading-none">
                  <span className="font-semibold text-slate-700 uppercase truncate max-w-[120px]">{item.name}</span>
                  <span className="font-semibold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full text-[9px]">{item.stock} Unit</span>
                </div>
              )) : <p className="text-[10px] text-slate-300 font-semibold py-4 uppercase">All Nodes Healthy</p>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="lg:hidden divide-y divide-slate-50">
              {salesData?.topPurchasedItems?.length > 0 ? salesData.topPurchasedItems.slice(0, 5).map((item: any, i: number) => (
                <div key={i} className="py-3 flex justify-between items-center">
                   <div>
                      <p className="text-[10px] font-semibold text-slate-800 uppercase">{item._id}</p>
                      <p className="text-[8px] font-semibold text-slate-400 uppercase">{item.totalQty} Units Inbound</p>
                   </div>
                   <p className="text-[10px] font-semibold text-indigo-600">₹{item.investment.toLocaleString()}</p>
                </div>
              )) : <p className="py-6 text-center text-slate-200 font-semibold text-[10px] uppercase">NULL_SET</p>}
            </div>
            ...
            <div className="hidden lg:block overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {salesData?.topPurchasedItems?.length > 0 ? salesData.topPurchasedItems.slice(0, 5).map((item: any, i: number) => (
                    <tr key={i} className="text-[11px] font-semibold text-slate-700 uppercase">
                    ...
                    <tr><td colSpan={3} className="py-10 text-center text-slate-300 font-semibold tracking-widest">NULL_SET</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="bg-slate-900 p-5 rounded-2xl border-2 border-slate-800 shadow-xl overflow-hidden">
          <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-3">
            ...
            <span className="text-[8px] font-semibold text-white/20 uppercase">Real-time Feed</span>
          </div>
          <div className="space-y-3 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
            {salesData?.activities?.length > 0 ? salesData.activities.map((act: any, i: number) => (
              <div key={i} className="flex flex-col gap-1 py-2 border-b border-white/5 last:border-b-0">
                <div className="flex justify-between items-center text-[10px] font-semibold uppercase tracking-widest leading-none">
                  <span className={act.action === 'CREATE' ? 'text-indigo-400' : 'text-amber-400'}>{act.resource} Node</span>
                  <span className="text-white/20 text-[8px] font-semibold">{new Date(act.createdAt).toLocaleTimeString()}</span>
                </div>
                <p className="text-[9px] text-white/50 font-semibold tracking-tight uppercase leading-none mt-1">{act.description}</p>
              </div>
            )) : <p className="py-10 text-center text-white/10 font-semibold text-[10px] tracking-widest">NO_LOGS_RECORDED</p>}
          </div>
        </div>
      ...
      <div className="bg-gradient-to-br from-indigo-900 to-slate-950 p-6 rounded-2xl border-2 border-white/10 shadow-2xl relative overflow-hidden mt-4">
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8">
           <div>
              <p className="text-[10px] font-semibold text-indigo-400 uppercase tracking-[0.2em] mb-2">Node A1: Discovery</p>
           ...
           <div className="md:border-x border-white/5 md:px-8">
              <p className="text-[10px] font-semibold text-emerald-400 uppercase tracking-[0.2em] mb-2">Node B2: Fiscal Health</p>
              <h4 className="text-white text-lg font-semibold tracking-tight">Promotional Impact Audit</h4>
              <div className="mt-3 space-y-2">
                 <div className="flex justify-between text-[9px] font-semibold text-slate-300 uppercase">
           ...
           <div>
              <p className="text-[10px] font-semibold text-rose-400 uppercase tracking-[0.2em] mb-2">Node C3: Optimization</p>
              ...
              <div className="mt-4 flex gap-2 no-print">
                 <button onClick={() => window.print()} className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl text-[8px] font-semibold uppercase tracking-widest transition-all">Download Audit PDF</button>
              </div>
           </div>
        </div>
      </div>
         ...
         <div className="flex flex-col">
               ...
               {salesData?.gstSlabs?.length > 0 ? salesData.gstSlabs.map((slab:any, i:number) => (
                 <div key={i} className="py-3 space-y-2">
                    <div className="flex justify-between items-center">
                       <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-widest">GST {slab._id}% Protocol</span>
                       <span className="text-[10px] font-semibold text-amber-600">₹{slab.totalTax.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                       <span className="text-[8px] font-semibold text-slate-300 uppercase tracking-widest">Taxable Value</span>
                       <span className="text-[10px] font-bold text-slate-700">₹{slab.taxableValue.toLocaleString()}</span>
                    </div>
                 </div>
               )) : <p className="py-6 text-center text-slate-200 font-semibold text-[10px] uppercase">NO_TAX_DATA_SYNCED</p>}
               {salesData?.gstSlabs?.length > 0 && (
                 <div className="pt-3 border-t-2 border-slate-100 mt-2">
                    <div className="flex justify-between items-center bg-slate-50 p-2 rounded-lg">
                       <span className="text-[9px] font-semibold text-slate-900 uppercase">Grand Aggregate</span>
                       <span className="text-xs font-semibold text-amber-600">₹{salesData.totalGST.toLocaleString()}</span>
                    </div>
                 </div>
               )}
            </div>
            ...
            <div className="hidden lg:block overflow-x-auto">
               <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                     {salesData?.gstSlabs?.length > 0 ? salesData.gstSlabs.map((slab:any, i:number) => (
                       <tr key={i} className="text-[10px] font-semibold text-slate-700 uppercase">
                          ...
                          <td className="py-3 px-2 text-right text-amber-600 font-semibold">₹{slab.totalTax.toLocaleString()}</td>
                       </tr>
                     )) : (
                       <tr><td colSpan={3} className="py-10 text-center text-slate-200 font-semibold tracking-widest">NO_TAX_DATA_SYNCED</td></tr>
                     )}
                  </tbody>
                  {salesData?.gstSlabs?.length > 0 && (
                    <tfoot>
                       <tr className="bg-slate-50/50 border-t-2 border-slate-100 font-semibold text-[10px] uppercase">
           ...
           <div className="flex justify-center gap-4 mt-4 pb-2">
               {[{p:"UPI",c:"#10b981"},{p:"CASH",c:"#6366f1"},{p:"CARD",c:"#f43f5e"}].map((proto) => (
                  <div key={proto.p} className="flex items-center gap-1.5">
                     ...
                     <span className="text-[9px] font-semibold uppercase text-slate-400">{proto.p}</span>
                  </div>
               ))}
           </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-4">
        ...
        <div className="lg:col-span-4 bg-white p-5 rounded-2xl border-2 border-slate-200 shadow-sm flex flex-col h-[300px] overflow-hidden">
           ...
           <p className="text-[8px] font-semibold text-slate-400 uppercase text-center mt-2 tracking-widest">Transaction Count Frequency</p>
        </div>
