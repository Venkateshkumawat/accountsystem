...
const AuditCenter: React.FC = () => {
    ...
    // Apply 10 items limit initially
    ...
    return (
        <div className="nexus-container py-8 animate-in fade-in duration-700 font-inter">
            {/* ── HEADER ────────────────────────────────────────────────────────── */}
            <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
                <div>
                    <h1 className="text-2xl font-semibold text-slate-900 tracking-tight flex items-center gap-3 font-inter">
                        <Bell className="text-indigo-600" size={24} /> Notifications
                    </h1>
                    <p className="text-[10px] font-semibold text-slate-400 mt-1 uppercase tracking-[0.2em] leading-none font-inter">Unified Activity Stream · v2.5</p>
                </div>

                <div className="flex items-center gap-3">
                    <button 
                        onClick={() => confirm("Clear all unread messages?") && deleteAllNotifications()}
                        className="px-6 py-2.5 bg-white border border-slate-100 text-[10px] font-semibold uppercase tracking-widest text-rose-600 rounded-2xl hover:bg-rose-50 transition-all font-inter"
                    >
                        Clear Messages
                    </button>
                    <div className="px-5 py-2.5 bg-slate-900 rounded-2xl flex items-center gap-4 text-white shadow-xl">
            ...
            <div className="relative mb-8">
                <input 
                    type="text" 
                    placeholder="Search recent notifications, staff actions, or transactions..."
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    className="w-full pl-12 pr-6 py-4 bg-white border border-slate-100 rounded-[1.5rem] text-[13px] font-semibold text-slate-600 placeholder:text-slate-300 focus:border-indigo-500 shadow-sm transition-all font-inter"
                />
                ...
                <table className="w-full text-left table-fixed">
                    <thead>
                        <tr className="border-b border-slate-50 bg-slate-50/30">
                            ...
                            <th className="w-[100px] px-8 py-5 text-[9px] font-semibold text-slate-400 uppercase tracking-[0.2em] font-inter">Actions</th>
                        </tr>
                    </thead>
                ...
                {!showAll && unifiedStream.length > 10 && (
                    <div className="p-8 bg-slate-50/50 flex justify-center border-t border-slate-50">
                        <button 
                            ...
                            className="flex items-center gap-3 px-8 py-3 bg-white border border-slate-200 rounded-2xl text-[12px] font-semibold text-slate-600 hover:text-indigo-600 hover:border-indigo-100 hover:shadow-lg hover:shadow-indigo-50 transition-all font-inter group"
                        >
                            <span>See All Messages ({unifiedStream.length - 10} remaining)</span>
            ...
            {/* ── FOOTER ANALYTICS ────────────────────────────────────────────────────────── */}
            <footer className="mt-8 flex justify-between items-center text-[9px] font-semibold text-slate-400 uppercase tracking-[0.2em] font-inter">
                <p>© 2026 Nexus Terminal · Secure Activity Log</p>
                <div className="flex items-center gap-6">
                    <span className="flex items-center gap-2 font-inter"><div className="w-1.5 h-1.5 bg-emerald-400 rounded-full" /> Live Sync</span>
                    <span className="flex items-center gap-2 font-inter"><Zap size={14} className="text-indigo-400 animate-pulse" /> Telemetry Active</span>
                </div>
            </footer>
        </div>
    );
