                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-[10px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100">
                                <th className="py-4 px-8 min-w-[300px]">Party Details</th>
                                <th className="py-4 px-8">Contact Node</th>
                                <th className="py-4 px-8 text-center">Protocol Type</th>
                                <th className="py-4 px-8 text-right">Settlement Balance</th>
                                ...
                                <tr key={party._id} className="group hover:bg-slate-50/50 transition-colors">
                                    <td className="py-4 px-8">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-400 font-bold group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600 transition-all cursor-pointer">
                                                {party.name.charAt(0)}
                                            </div>
                                            <div className="min-w-0">
                                                <h4 className="text-sm font-semibold text-slate-900 tracking-tight truncate leading-none mb-1.5">{party.name}</h4>
                                                <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                                                    <ShieldCheck size={10} className="text-slate-400" />
                                                    <span className="text-[9px] font-bold uppercase tracking-widest text-slate-500">{party.gstin || 'NO GST REG'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                        ...
                                        <div className="space-y-1.5">
                                            <div className="flex items-center gap-3">
                                                <div className="flex items-center gap-2 text-[11px] font-bold text-slate-800">
                                                    <Phone size={10} className="text-slate-400" /> +91 {party.phone}
                                                </div>
                                                <div className="flex items-center gap-1.5 no-print">
                                                    <a href={`tel:${party.phone}`} className="p-1 px-2 bg-indigo-50 text-indigo-600 rounded-md hover:bg-indigo-600 hover:text-white transition-all">
                                                        <Phone size={8} />
                                                    </a>
                                                    <a href={`https://wa.me/91${party.phone}`} target="_blank" rel="noreferrer" className="p-1 px-2 bg-emerald-50 text-emerald-600 rounded-md hover:bg-emerald-600 hover:text-white transition-all">
                                    ...
                                    <td className="py-4 px-8 text-right">
                                        <div className="flex flex-col items-end">
                                            <span className={`text-[13px] font-bold tracking-tight ${(party.currentBalance || 0) >= 0 ? ((party.type === 'Vendor' || party.type === 'Supplier') ? 'text-rose-600' : 'text-emerald-600') : 'text-slate-400'}`}>
                                                ₹{Math.abs(party.currentBalance || 0).toLocaleString()}
                                            </span>
                                            <span className="text-[8px] font-bold uppercase tracking-widest text-slate-300 mt-0.5">
                                                {(party.type === 'Vendor' || party.type === 'Supplier') ? 'TO PAY' : 'TO RECEIVE'}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="py-4 px-8 text-right">
                                        <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all">
                                            <button 
                                                onClick={() => { setSelectedParty(party); setShowPaymentModal(true); }}
                                            ...
                                            <div className="relative group/menu">
                                                ...
                                                <div className="absolute right-0 top-full mt-1 w-36 bg-white border border-slate-100 shadow-2xl rounded-xl invisible group-hover/menu:visible opacity-0 group-hover/menu:opacity-100 transition-all z-20 overflow-hidden">
                                                    <button onClick={() => { setEditingNode(party); setFormData(party); setShowModal(true); }} className="w-full text-left px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 flex items-center gap-2">
                                                        <Edit2 size={12} /> Edit Node
                                                    </button>
                                                    <button onClick={() => handleDelete(party._id)} className="w-full text-left px-4 py-2 text-[10px] font-bold text-rose-500 hover:bg-rose-50 flex items-center gap-2 border-t border-slate-50">
