...
export default function Parties() {
    ...
    const fetchPartyHistory = async (party: any) => {
        ...
        try {
            ...
            const documents = (commRes.data?.data || []).map((doc: any) => ({
                id: doc._id,
                title: doc.invoiceNumber || doc.purchaseNumber || 'Document',
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Remove this partner from the ledger?')) return;
        try {
            ...
            sync.postMessage('SYNC_PARTIES');
            sync.close();
        } catch (err) { notifyError('Deletion Failed'); }
    };
    ...
    // Arrange partners within the selected filter scope
    const filteredParties = useMemo(() => {
        ...
        // Sort partners by name or balance amount
        ...
        <div className="p-2 sm:p-4 space-y-4 bg-[#fcfcfd] min-h-screen font-inter select-none">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 px-1 no-print">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl sm:text-2xl font-semibold text-slate-900 tracking-tight">Business Partners</h1>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">CRM & Fiscal Ledger</p>
                    </div>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                        <Plus size={14} /> <span className="hidden xs:inline">Add</span> Partner
                    </button>
            ...
            <div className="bg-white rounded-3xl sm:rounded-[2.5rem] border-2 border-slate-200 shadow-sm overflow-hidden min-h-[500px]">
                    ...
                    <table className="w-full text-left border-collapse table-fixed">
                        <thead>
                            <tr className="text-[10px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100 bg-slate-50/50">
                                <th className="py-4 px-3">Partner</th>
                    ...
                    {(showAllParties || search || activeFilter !== 'All' ? filteredParties : filteredParties.slice(0, 10)).map((party) => (
                        <div key={party._id} className="p-5 space-y-4 border-b-2 border-slate-100 last:border-0 hover:bg-indigo-50/30 transition-all duration-300">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="bg-slate-50/50 p-2.5 rounded-xl border border-slate-100">
                                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">GSTIN</p>
                                    <p className="text-[10px] font-bold text-slate-600 truncate">{party.gstin || 'N/A'}</p>
                                </div>
                            </div>
                ...
                {filteredParties.length === 0 && (
                    <div className="py-24 text-center px-4">
                        ...
                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">No matching partners found</p>
                    </div>
                )}
                {filteredParties.length > 10 && !search && activeFilter === 'All' && (
                    <div className="p-4 sm:p-6 bg-slate-50 border-t border-slate-100/50 text-center">
                        ...
                        >
                            <History size={14} />
                            {showAllParties ? 'SEE LESS' : `SEE ALL (${filteredParties.length})`}
                        </button>
                    </div>
                )}
            </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1.5 flex items-center gap-1.5">
                                        <MapPin size={10} className="text-indigo-500" /> {selectedParty.address?.city || 'Location Unknown'}
                                    </p>
                                </div>
                            </div>
                            <button 
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-8 space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                                    ...
                                    <p className="text-[7px] font-bold text-slate-400 uppercase mt-1">Outstanding Balance</p>
                                </div>
                                <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                                    ...
                                    <p className="text-[7px] font-bold text-slate-400 uppercase mt-1">Primary Number</p>
                                </div>
                            </div>
                                ...
                                <div className="space-y-3">
                                    {historyLoading ? (
                                        <div className="py-12 text-center animate-pulse">
                                            ...
                                            <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em]">Loading History...</p>
                                        </div>
                                        ...
                                        <div className="py-20 text-center bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-100">
                                            ...
                                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">No transactions logged</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4">
                    <div className="bg-white w-full max-w-2xl rounded-t-[2rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom sm:zoom-in-95 duration-200 border border-slate-100 flex flex-col max-h-[95vh] sm:max-h-[90vh]">
                        <header className="px-6 sm:px-8 py-5 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                            <div>
                                <h3 className="text-base sm:text-lg font-bold text-slate-900 uppercase tracking-tight">{editingNode ? 'Edit Partner' : 'Add Partner'}</h3>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Directory Management</p>
                            </div>
                        ...
                        <form onSubmit={handleSave} className="p-6 sm:p-8 space-y-6 overflow-y-auto custom-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <FormItem label="Partner Name" icon={Users} value={formData.name} onChange={(v: string) => setFormData({...formData, name: v})} required />
                                <FormItem label="Contact" icon={Phone} value={formData.phone} onChange={(v: string) => setFormData({...formData, phone: v})} required maxLength={10} />
                                <FormItem label="Type" icon={Activity} isSelect>
                                    <div className="flex gap-2">
