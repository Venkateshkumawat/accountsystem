...
export default function SuperAdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F9] flex font-['Plus_Jakarta_Sans','Outfit',sans-serif]">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="font-mono text-[14px] font-black text-slate-900">{provisioningResult.businessId}</span>
                      ...
                      <div className="flex justify-between items-center p-6 bg-slate-900 rounded-[1.5rem]">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="font-mono text-[18px] font-black text-white">{provisioningResult.loginCredentials.password}</span>
              ...
              {plans.map((plan, i) => (
                <div key={i} className="glass-card p-10 group relative overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl">
                  ...
                  <div className="space-y-4 mb-8">
                    <div className="flex items-center justify-between text-[11px] font-black text-slate-600 border-b border-slate-100 pb-3"><span className="uppercase text-slate-400 tracking-widest">SKU Quota</span><span className="text-slate-900">{plan.maxProducts} units</span></div>
...
function SystemPipe({ label, percent, color, isValueOnly }: any) {
  return (<div className="group"><div className="flex items-center justify-between mb-2"><p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] group-hover:text-slate-900 transition-colors">{label}</p><span className="text-[12px] font-black text-slate-900 uppercase font-mono">{percent}</span></div>{!isValueOnly && (<div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden p-0.5"><div className={`h-full ${color} rounded-full transition-all duration-[2s] ease-out group-hover:brightness-110 shadow-sm`} style={{ width: percent }} /></div>)}</div>);
}
